...
def crop_objects(img, data, path, allowed_classes):
    ...
    counts = dict()
    print("count of objects for image name", counts)

    roi_player = []
    for i in range(num_objects):
        # get count of class for part of image name
        class_index = int(float(classes[i]))
        class_name = class_names[class_index]
        if class_name in allowed_classes:
            counts[class_name] = counts.get(class_name, 0) + 1
            print("boxes:", boxes)
            ...
            # print("this is the image from the cropped image:",im2arr)
            ...
            print(img_path)
            # save image
            cropped_image = cv2.imwrite(img_path, cropped_img)
